...
binomial 0 k 
    | k > 0     = 0
    | otherwise = error "k must be positive"
...
countdown n = "Ready! " ++ (countdown_helper n)
    where 
        countdown_helper 0 = "Liftoff!"
        countdown_helper n = (show n) ++ "... " ++ countdown_helper (n - 1)
...
smallestDivisor x = smallestDivisor_helper x 2
    where
        smallestDivisor_helper x y
            | y == x       = x
            | mod x y == 0 = y
            | otherwise    = smallestDivisor_helper x (y + 1)
